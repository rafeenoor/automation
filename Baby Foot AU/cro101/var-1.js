...
// ───────────────────────────────────────────────────────────────────────────────
app.view('crp_create_commit', async ({ ack, body, client, view }) => {
  const { clientKey, testName, count } = JSON.parse(view.private_metadata);
  const cfg = CLIENTS[clientKey];

  // 1) ACK immediately with a loading modal so Slack doesn't time out
  await ack({
    response_action: 'update',
    view: {
      type: 'modal',
      callback_id: 'noop_loading',
      title: { type: 'plain_text', text: 'Creating…' },
      close: { type: 'plain_text', text: 'Cancel' },
      blocks: [
        { type: 'section', text: { type: 'mrkdwn', text: `Creating *${testName}* for *${clientKey}*…` } }
      ]
    }
  });

  // 2) do the GitHub writes
  try {
    const ops = [];
    for (let i = 1; i <= count; i++) {
    ...
    for (const op of ops) {
      // get sha if exists (idempotent)
      let sha = null;
      try {
        const r = await octokit.repos.getContent({ owner: cfg.owner, repo: cfg.repo, path: op.path, ref: DEFAULT_BRANCH });
        if (!Array.isArray(r.data)) sha = r.data.sha;
      } catch (_) {}
      const b64 = Buffer.from(op.content || '', 'utf8').toString('base64');
      await octokit.repos.createOrUpdateFileContents({
        owner: cfg.owner, repo: cfg.repo, path: op.path,
        message: `CRP: create ${testName} (${clientKey}) -> ${op.path}`,
        content: b64, branch: DEFAULT_BRANCH, sha: sha || undefined
      });
    }

    // 3) update the same modal with success
    await client.views.update({
      view_id: body.view.id,
      view: {
        type: 'modal',
        callback_id: 'noop_done',
    ...
    // 4) show error in the same modal
    await client.views.update({
      view_id: body.view.id,
      view: {
        type: 'modal',
        callback_id: 'noop_err',
        title: { type: 'plain_text', text: 'Error' },
        close: { type: 'plain_text', text: 'Close' },
        blocks: [
          { type: 'section',
            text: { type: 'mrkdwn', text: `❌ GitHub error creating files:\n\`${e.message}\`` } }
        ]
      }
    });
  }
...
// --- update flow: same pattern ---
app.view('crp_update_commit', async ({ ack, body, client, view }) => {
  const { clientKey, testName } = JSON.parse(view.private_metadata);
  const cfg  = CLIENTS[clientKey];
  ...
  await ack({
    ...
    view: {
      type: 'modal',
      callback_id: 'noop_loading',
      title: { type: 'plain_text', text: 'Updating…' },
      close: { type: 'plain_text', text: 'Cancel' },
      blocks: [{ type: 'section', text: { type: 'mrkdwn', text: `Updating *${testName}* variation *${v}*…` } }]
    }
  });
